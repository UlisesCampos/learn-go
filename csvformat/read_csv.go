...
func ReadCSV(b io.Reader) ([]Movie, error) {
	...
	var movies []Movie
	// grab and ignore the header for now we may also want to use this for a dictionary key
	// or some other form of lookup
	_, err := r.Read()
...
func AddMoviesFromText() error {
	// this is an example of us taking a string, converting it into a buffer, and
	// reading it with the csv package
	in := `
- first our headers
